(ns mtdm.classifier.distributed.dynamic-distributed
...
(defn- get-potential-t-site-source-sites
  ...
  source-sites."
  [sites shared-sources? best-pairs]
...
;; new code starts
 best-pairs-items
        (loop [x best-pairs
          ...
          (if (= (empty? x) false)
            ;;(println "check" (first x))
            (recur (rest x) (conj y (ffirst x) (second (first x))))
            ;;(println "check" y)
            y
            )
        ...
        see
         (for [ x best-pairs
                                        y (get-all-source-sites sites) :when (= (some #(= (first x) %) y) nil) ] x)
        
        see1 (loop [x best-pairs
                    y ()
                    ]
               ;;(def z (atom 0))
               (if (= (empty? x) false)
                 (do
                   (let [z (cond
                             (= (some #(= (ffirst x) %) (get-all-source-sites sites)) nil)
                             (first x)
                             :else ()
                             ) ] 

                     (recur (rest x) (conj y z)) 
                     )
                   )
                 (filter #(not= % ()) y) 
                 )
...
;; changed code starts here
      (-> (cond
(= (get-all-source-sites sites) ())
(do
(-> (set all-site-indexes) 
       
        (difference (set best-pairs-items))   
...
(defn- get-creation-monitors
  ...
  ([sites trouble-factor t-site-input-type shared-sources?
    creation-monitor-factory best-pairs]
   (get-creation-monitors sites trouble-factor t-site-input-type
                          shared-sources? creation-monitor-factory best-pairs {} []))
  ([sites trouble-factor t-site-input-type shared-sources?
    creation-monitor-factory best-pairs previous-monitors blacklisted-source-sites]
...
(defn- create-t-sites!
  ...
  (let [;; Find source-sites where the monitor's criteria for creating
        ...
        candidate-source-sites (->> creation-monitors
                                    ...
                                    (reverse)
                                    (map first))
  cand-time-value  (first (map read-string (re-seq #"[\d.]+" (with-out-str (time (->> creation-monitors
                                                                      (filter #(fire? (second %)))
                                                                      (sort-by #(get-proportion (second %)))
                                                                      (reverse)
                                                                      (map first))
    ...
    [new-site-indexes  cand-time-value]
     ;;   new-site-indexes
        ))))
...
(defn dynamic-distributed-classifier
  ...
  [base-site-structure schema classifier-generator moving-average-generator
   ...
   & {:keys [disable-monitors disable-monitor-logging best-pairs]
      :or {disable-monitors false
           disable-monitor-logging false best-pairs () }}]
(def time-atom (atom 0))
 (def counter (atom 0))
(def cand-time (atom #{}))
(def update-time (atom #{}))
(def cand-time1 (atom 0))
(def update-time1 (atom 0))
...
time-cpu (- (:cpu-nano stop-time)
                                         (:cpu-nano start-time))


time-wall (- (:wall-nano stop-time)
                                         (:wall-nano start-time))


no-of-pairs (count (get-creation-monitors
                     (get-sites classifier)
                     trouble-factor
                     t-site-input-type
                     shared-sources?
...
time-func (first (map read-string (re-seq #"[\d.]+" (with-out-str (time (get-creation-monitors
                                  (get-sites classifier)
                                  trouble-factor
                                  t-site-input-type
                                  shared-sources?
    ...
    (reify
      ...
      (process-record [this record]
        (let [result (process-record classifier record)
              result (-> result
                      (assoc :time-cpu-pairs time-cpu)
                      (assoc :time-wall-pairs time-wall)
                      (assoc :total-pairs no-of-pairs)
                      (assoc :time-func time-func)
                      )
...
;; (swap! update-time conj [(:id record) (first (map read-string (re-seq #"[\d.]+" (with-out-str (time (swap! creation-monitors-atom
...
(reset! update-time1 (first (map read-string (re-seq #"[\d.]+" (with-out-str (time (swap! creation-monitors-atom
                                                                                                             #(update-potential-monitors % result (get-sites this)
...
(swap! update-time conj [(:id record) @update-time1])
            ...
            (let [;; Create and remove t-sites.
                 ;; created-site-indexes
  [created-site-indexes  cand-time-value] 
                  (create-t-sites! sites-atom
  ...
  (reset! cand-time1 cand-time-value)

(if (and (= @time-atom 0) (= created-site-indexes []))
(do
  (swap! cand-time conj [(:id record)  cand-time-value])
...
(if (and (= @time-atom 0) (not= created-site-indexes []))
(do
;;(println (:id record))
;;(println (count @cand-time) " " @cand-time)
;;(println "sec" (count (map #(get % 1)  @cand-time) ) (apply + (map #(get % 1)  @cand-time) ) )

;;(println (count @update-time) " " @update-time)
;;(println "sec" (count (map #(get % 1)  @update-time) ) (apply + (map #(get % 1)  @update-time) ) )

(reset! time-atom (+ (apply + (map #(get % 1)  @cand-time))  (apply + (map #(get % 1)  @update-time) ))
 )
;;(println "time" @time-atom)
...
(->> result
        ...
        (-> result
        (assoc :time-tsite @time-atom)
   (assoc :time-agree (+ @update-time1 @cand-time1)
;;(+ (get @cand-time1 1) (get @update-time 1))
              )
))
