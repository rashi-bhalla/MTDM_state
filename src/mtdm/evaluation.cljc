...
(defn get-primary-site-communication
  "Return a map of site indexes associated with maps of their
  source-sites and the proportion of the total dataset's records they
  sent to the trouble-site."
  [results no-of-records sites]
;;(println "sites" sites)
 (let  [psites (->> sites
...
;;(println "sites" psites)
  (->> psites
     (map #(hash-map % no-of-records))
      (into (sorted-map))
       (map-vals #(/ % (count results)))
       (into {})
       )
...
(defn primary-weight-communication-to-site
  "Alter the communication-to-site proportions to reflect the
  feature-widths/weights of different sites."
  [primary-comm site-weights]
;; (println "keys inside" (first primary-comm))
(let [source-site-index (first primary-comm)
      record-proportion (second primary-comm)
 ]
       
  (hash-map source-site-index (* record-proportion  (get site-weights source-site-index)))   
))
...
(defn weight-primary-communication
  "Alter the communication-to-sites proportions to reflect the
  feature-widths/weights of different sites."
  [primary-comm sites]
  (let [
...
psite-weights (get-site-weights psites)
]
(->> primary-comm
(map #(primary-weight-communication-to-site % psite-weights) ) 
(into (sorted-map))
)))
...
(let [;;pretty-comm  (pretty-site-communication site-communication sites percent)
   ...
   normalized-comm (for [x  site-communication
...
(defn total-primary-site-communication 
[primary-site-communication]
...
(defn get-time-agree
[results]
;;(println "map" (map #(:time-agree %) results) )
(->> results
     (map #(:time-agree %)) 
     (reduce +)
)
...
(defn get-primary-proportion-transmitted
  [primary-weighted-site-communication]
  (->> primary-weighted-site-communication
       (vals)
    ;;  (map #(reduce + (vals %)))
      (reduce +)
...
(defn temporal-summary
  [min-records site-structure all-results site-changelog partition-size
   ]
;;(println "temp")
  (->> all-results
        ...
        (fn [results]
          (let [proportions-at-p-sites
            ...
            {:results-count (count results)
             ...
             :proportion-transmitted (-> results
  ...
  :time-update-agree (get-time-agree results)
...
(defn results-summary
  ([min-records site-structure results site-changelog
    & {:keys [pretty? skip-records time-cal time-cpu-pairs time-wall-pairs total-pairs time-func time-tsite
              ]
       :or {pretty? true
            skip-records [1000]}}]
;;(println "min inside" min-records)
;;(println "res")
   ...
   time-update-agree (get-time-agree results)
         primary-site-communication (get-primary-site-communication results min-records sites)
         total-communication (total-site-communication site-communication)
;; (+ (total-site-communication site-communication) (total-primary-site-communication primary-site-communication)) 
        full-comm (get-full-transmission site-communication)
         prim-comm (total-primary-site-communication primary-site-communication)
         weighted-primary-site-communication (weight-primary-communication primary-site-communication sites)
         ...
         site-hit-rates (->> (vals sites)
                             ...
                             (into (sorted-map)))
         temporal-summary-100 (temporal-summary min-records site-structure results site-changelog 100 )
]
;(println "site" site-communication)
;;(println "total" total-communication)
;;(println "full" full-comm)
;;(println "prim" prim-comm)
;;(println "min" min-records)
;;(println "prim1" primary-site-communication)
;;(println "prim" prim-comm)
     {:results-count results-count
      :accuracy (percent (get-accuracy results))
      :proportion-transmitted (percent (+ (get-proportion-transmitted weighted-site-communication) (get-primary-proportion-transmitted weighted-primary-site-communication)) )
      :site-hit-rates site-hit-rates
      :site-communication (pretty-site-communication site-communication sites percent)
      :total-communication ;;add percent
      (+ (total-site-communication site-communication) (total-primary-site-communication primary-site-communication)) 
      :prim-comm (total-primary-site-communication primary-site-communication)
      :weighted-site-communication (pretty-site-communication weighted-site-communication sites percent)
      :full-comm (+ (get-full-transmission site-communication) (total-primary-site-communication primary-site-communication))
      ...
      (zipmap skip-records
              (for [skip-count skip-records]
                (results-summary min-records site-structure
