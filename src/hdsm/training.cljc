...
;; [hdsm.classifier.distributed.dynamic-distributed :refer [time-atom]]
...
time-tsite (:time-tsite (first (filter #(not= (:time-tsite %) 0) results)) )

]
;;(println (first results))
;;(println "time-tsite" time-tsite)
;;(reset! time-atom {:time-tsite 0})
;;(println "time-tsite after" time-tsite)
     ...
     {:label label
      ...
      :summary (results-summary label sites results (get-site-changelog classifier)
                                :pretty? false
                                :time-cal ttime
