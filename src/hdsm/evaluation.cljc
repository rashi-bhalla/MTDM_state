...
(defn results-summary
  ([label site-structure results site-changelog
    & {:keys [pretty? skip-records time-cal time-cpu-pairs time-wall-pairs total-pairs time-func time-tsite]
       :or {pretty? true
            skip-records [1000]}}]
;;(println "time inside" (type time-func) "pairs" ())
;;(println "in" time-tsite "  label" label)
   (let [sites (if (map? site-structure)
         ...
         site-hit-rates (->> (vals sites)
                             ...
                             (into (sorted-map)))
            total-pairs  (if (clojure.string/includes? label "naive") 0 total-pairs)
         temporal-summary-100 (temporal-summary site-structure results site-changelog 100)]
;;(println "total" time-update-agree)
;;(println "full" full-comm)
     ...
     {:results-count results-count
      ...
      :total-communication (total-site-communication site-communication)
      :full-transmission (get-full-transmission site-communication)
   ...
   :time-update-agree (get-time-agree results)
   :time-tsite (if (= (get-time-agree results) 0) 0 time-tsite)
    :time-func (if (= (get-time-agree results) 0) 0 time-func) 
  ;;    :full-trans (percent (get-full-transmission site-communication))
      :subset-summaries
      (zipmap skip-records
              (for [skip-count skip-records]
                (results-summary label site-structure
...
;;(println "full-trans" (percent (get-full-transmission site-communication)))
)))
