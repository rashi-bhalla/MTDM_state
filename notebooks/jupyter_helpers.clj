...
(defn site-summary-table
  ...
  (let [summary (results-summary (:label experiment) (:final-site-structure experiment)
                                 (drop skip-records (:results experiment))
                                 (:site-changelog experiment)
                                 :pretty? false
                                 :skip-records [] 
                                 :time-cal (:time-cal experiment)  
                                 :time-cpu-pairs (:time-cpu-pairs (:results experiment))
        ...
        total-row (format-site-summary-row "Totals" 1 (:accuracy summary)
                                           ...
                                           (:full-transmission summary)
                                           (:time-update-agree summary)
)
...
;;                                                  (zipmap (keys other-experiments)))
        ...
        ;; full-plot (doto
        ;;                    (Plot.)
        ;;                  (.setYLabel "Full transmission")
        ;;                  (.setLegendPosition LegendPosition/TOP)
        ;;                  (.setLegendLayout LegendLayout/HORIZONTAL)
        ;;                  (.setXBound (first x-bounds) (second x-bounds))
        ;;                  (.setYBound (first y-bounds-full) (second y-bounds-full)))
 ...
 ;;            (.setColor (Color/decode "#f55774"))
 ;;            (.setX x-values)
 ;;            (.setY (map (comp float :full-transmission)
 ;;                        experiment-temporal-summary))
 ;;            (.setDisplayName "Full Transmission")))
  ...
  ([output-dir base-classifier experiments & {:keys [beaker-output thread-count]
   ...
   (let [beaker (NamespaceClient/getBeakerX)
         ...
         summary-keys [:proportion-transmitted :mean-max-transmission-100
                       :accuracy :f-score :mean-critical-path-time :time-cal :time-cpu-pairs :time-wall-pairs :total-pairs :time-func :total-communication :full-transmission :time-tsite :time-update-agree]]
...
(defn experiments-summary-table
 ...
 ;;   ["Full time cal" #(float (:time-cal %))]
                ["Full Total Transmission" #(percent (-> % :full :proportion-transmitted))]
                  ["Full Transmission" #(percent (-> % :full :full-transmission))]
...
;; ["Full time-func" #(float (:time-func %))]
  ...
  ;;              ["Skip-1000 time cal" #(float (-> % :skip-1000 :time-cal))]
                ["Skip-1000 Total Transmission" #(percent (-> % :skip-1000 :proportion-transmitted))]
                   ["Skip-1000 Transmission" #(percent (-> % :skip-1000 :full-transmission))]
