(ns jupyter-helpers
  (:require [clojure.string :as string]
            [mtdm.utils :refer [map-vals percent
                                                         seq-contains? *debug*
                                                         safe-division]]
            [mtdm.trees.graph-tree :refer [model->dot]]
            [mtdm.training
...
(defn site-summary-table
  [experiment
   ...
   ;; (println "check" (:time-cal experiment)
  (let [summary (results-summary (:min-records experiment) (:final-site-structure experiment)
                                 (drop skip-records (:results experiment))
                                 (:site-changelog experiment)
                                 :pretty? false
                                 :skip-records [] 
                                 :time-cal (:time-cal experiment)                                       :time-cpu-pairs (:time-cpu-pairs (:results experiment ))
                                 :time-wall-pairs (:time-wall-pairs (:results experiment))
                                 :total-pairs (:total-pairs (:results experiment))

                                 :time-func (:time-func (:results experiment))

 )
  
        total-row (format-site-summary-row "Totals" 1 (:accuracy summary)
                                           (:proportion-transmitted summary)
(:total-communication summary)
(:full-comm summary)
(:prim-comm summary)
)
...
(defn experiment-timeline
  [experiment other-experiments &
   {:keys [partition-size extra-events
           x-bounds y-bounds-acc y-bounds-data y-bounds-agree
           event-rows
  ...
  (let [base-colour (Color/decode "#809AE5") ;; https://www.figma.com/file/KePb0YwhpKu45pXZnN9s4TmS/Accessible-Colors-for-Data-Visualization?node-id=0%3A1
        ...
        ;;                     other-experiments)
        experiment-temporal-summary (temporal-summary (:min-records experiment) (:final-site-structure experiment)
                                                      (:results experiment)
                                                      (:site-changelog experiment)
                                                    ;;  (:time-cal experiment)
                   ;;                                (:time-cpu-pairs (:results experiment))
                     ;;                              (:time-wall-pairs (:results experiment))
                       ;;                            (:total-pairs (:results experiment))
                                                   
                         ;;                         (:time-func (:results experiment))

                                                   
                                                      partition-size)
       ;;  other-experiment-temporal-summaries (->> other-experiments
 ;;                                                 (vals)
 ;;                                                 (map #(temporal-summary (:min-records %) (:final-site-structure %)
 ;;                                                                         (:results %)
 ;;                                                                         (:site-changelog %)
 ;;                                                                       ;;  (:time-cal %)
 ;;  ;; (:time-cpu-pairs (:results %)) 
 ;; ;; (:time-wall-pairs (:results %)) 
 ;; ;; (:total-pairs (:results %)) 
 ;; ;;  (:time-func (:results %)) 
 ;;                                                                         partition-size))
 ;;                                                 (zipmap (keys other-experiments)))
    ...
    time-update-plot (doto
                           (Plot.)
                         (.setYLabel "Time updating agreement")
                         (.setLegendPosition LegendPosition/TOP)
                         (.setLegendLayout LegendLayout/HORIZONTAL)
                         (.setXBound (first x-bounds) (second x-bounds))
                         (.setYBound (first y-bounds-agree) (second y-bounds-agree)))
 ...
 (.add time-update-plot
          (doto
              (Line.)
            (.setColor (Color/decode "#f55774"))
            (.setX x-values)
            (.setY (map (comp float :time-update-agree)
                        experiment-temporal-summary))
            (.setDisplayName "Time updating the agreement")))
    ...
    (doseq [[event-line event-text] (->> (concat (:site-changelog experiment)
      ...
      (.add acc-plot event-text)
        (.add time-update-plot event-line)
)
    comb-plot))
  ...
  ([output-dir base-classifier experiments & {:keys [beaker-output thread-count]
   ...
   (let [beaker (NamespaceClient/getBeakerX)
         ...
         summary-keys [:proportion-transmitted :mean-max-transmission-100
                       :accuracy :f-score :mean-critical-path-time :time-cal :time-cpu-pairs :time-wall-pairs :total-pairs :time-func :total-communication :full-comm :prim-comm :time-tsite :time-update-agree
                       ]]
...
(defn experiments-summary-table
  ...
  (let [fields [["Label" #(str (:label %))]
                ...
                ["Full Transmission" #(percent (-> % :full :full-comm))]
                ["Full pairs" #(int (-> % :full :total-pairs))]
                  ["Time taken" #(double (-> % :full :time-func))]
                ["Time taken for first tsite" #(double (-> % :full :time-tsite))]
...
;; ["Full time cpu cal" #(int (:time-cpu-pairs %))]
;; ["Full time wall cal" #(int (:time-wall-pairs %))]
;; ["Full pairs" #(int (-> % :full :total-pairs))]
;; ["Full time-func" #(float (:time-func %))]

                ;;["Full Micro-F" #(percent (-> % :full :f-score :micro))]
;;                ["Skip-1000 time cal" #(float (-> % :skip-1000 :time-cal))]
...
;;:distributed-ensemble (load-experiment output-dir (get-best-experiment-label experiments-summary performance-measure (str "naive" aggregation-method)))
   :mtdm (load-experiment output-dir (get-best-experiment-label experiments-summary performance-measure (str "mtdm" aggregation-method)))})
